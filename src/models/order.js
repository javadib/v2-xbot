...
بعد از تایید پرداخت، کانفیگ براتون توسط ربات ارسال میشه 🙏`,
    ...
    adminNewOrder(tUser, sPlan, sPayment, message) {
        ...
        💰مبلغ پرداختی: ${sPlan.totalPrice.toLocaleString()} تومان
         ...
         ℹاطلاعات واریز: \n${message.text}`;
        msg += '\n این اطلاعات بمزله پرداخت نیست. لطقعا بعد بررسی اطلاعات پرداخت، آن را تائید یا رد کنید 🙏'
    ...
    reviewInvoice(sPlan, sPayment) {
        let msg = `📦 نام پلن: ${sPlan?.name}
        
        💎 قیمت : ${sPlan?.totalPrice.toLocaleString()} 
              
        🔰  ${sPayment?.appKey} - ${sPayment?.appSecret}
        
        ♻️ لطفا مشخصات پرداخت رو در یک پیام ارسال کنید: 
        
        ✅ بعد از تایید پرداخت، کانفیگ به صورت خودکار ربات براتون ارسال میشه!`
